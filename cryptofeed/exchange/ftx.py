...
class FTX(Feed):
            ...
            for pair in pairs:
                ...
                if 'result' in data:
                    ...
                    if oi != self.open_interest.get(pair, None):
                        ...
                        await asyncio.sleep(1)  # don't fetch too many pairs too fast
    ...
    async def message_handler(self, msg: str, conn, timestamp: float):
        ...
        if 'type' in msg and msg['type'] == 'subscribed':
            return
        elif 'type' in msg and msg['type'] == 'info':
            LOG.info("%s: Info message received %s", self.conn_id, msg)
            return
        elif 'channel' in msg:
            if msg['channel'] == 'orderbook':
                ...
                LOG.warning("%s: Invalid message type %s", self.conn_id, msg)
        else:
            LOG.warning("%s: Invalid message type %s", self.conn_id, msg)
